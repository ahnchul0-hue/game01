...
import { BgmSynthesizer } from './BgmSynthesizer';
...
export class SoundManager {
    ...
    private bgmSynth = new BgmSynthesizer();
    ...
    playBgm(name: BgmName): void {
        ...
        this.stopBgm();
        // bgm-onsen은 노이즈 기반이므로 레거시 유지
        if (name === 'bgm-onsen') {
            this.createBgmOnsen();
            return;
        }
        // FM 합성 기반 BGM (BgmSynthesizer 위임)
        const nodes = this.bgmSynth.create(name, this.ctx, this.bgmGain);
        if (nodes) {
            this.bgmNodes = nodes;
        }
    }
    ...
    /** Bandpass-filtered looping white noise for water ambience (온천 화면 전용). */
